...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated isNetwork:(BOOL)isNetwork;
{
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    [view addSubview:hud];
    [hud show:animated];
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 95/3, 95/3)];
        imgView.image = [UIImage imageNamed:@"mb_progress_custom.png"];
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
        ...
        ((UIActivityIndicatorView *)indicator).color = kColorMainColor;
		...
		[self addSubview:indicator];
        [self runSpinAnimationOnView:self.customView duration:2.0 rotations:1 repeat:100000];
